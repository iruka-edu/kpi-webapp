...
export default function EmployeeProposal({ data, onChange, readonly = false }: EmployeeProposalProps) {
  ...
  return (
    <div className="space-y-4">
      {fields.map(f => (
        <div key={f.key} className="bg-slate-900/50 rounded-xl p-4 border border-slate-700/40">
          ...
          {readonly ? (
            <p className="text-slate-400 text-sm leading-relaxed whitespace-pre-wrap">
              {data[f.key] || <span className="italic text-slate-600">Không có đề xuất</span>}
            </p>
          ) : (
            <textarea
              rows={3}
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-white placeholder-slate-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none resize-none text-sm transition"
            />
          )}
        </div>
      ))}
