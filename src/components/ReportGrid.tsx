/**
 * ReportGrid.tsx — Lưới báo cáo KPI (2 Phân vùng)
 * --------------------------------------------------
 * Vai trò: Hiển thị 2 phân vùng:
 *
 *   PHÂN VÙNG 1 — BÁO CÁO TUẦN TRƯỚC (reportWeek):
 ...
 *
 *   PHÂN VÙNG 2 — KẾ HOẠCH TUẦN NÀY (planWeek):
 *     • Luôn trống, NV điền tự do
 *
 *   Cả 2 phân vùng hiển thị: Tuần X (DD/MM - DD/MM)
 */
...
type Props = {
  ...
  reportWeek: string;       // VD: "Tuần 15" - tuần đang báo cáo (phân vùng 1)
  planWeek: string;         // VD: "Tuần 16" - tuần sắp tới (phân vùng 2)
...
/**
 * Tiêu đề phân vùng chuẩn:
 * BÁO CÁO TUẦN 15 (07/04 - 13/04)
 * KẾ HOẠCH TUẦN 16 (14/04 - 20/04)
 */
function SectionHeader({ icon, label, weekStr, dateRange, hint, colorClass }: {
  icon: string;
  label: string;
  weekStr: string;
  dateRange: string;
  ...
  return (
    <tr>
      <td colSpan={10} className={`border border-gray-300 p-0`}>
        <div className={`flex flex-wrap items-center gap-2 px-4 py-3 ${colorClass}`}>
...
export default function ReportGrid({
  ...
  // Lần đầu: tự seed 1 dòng trống cho phân vùng 1
    ...
    <div className="w-full overflow-x-auto pb-24">
      <table className="w-full border-collapse border border-gray-300 text-sm">
        ...
        <tbody>
          ...
          {/* ══════════ PHÂN VÙNG 1: BÁO CÁO TUẦN TRƯỚC ══════════ */}
          <SectionHeader
            icon="📋"
            label="Báo cáo"
            weekStr={reportWeek}
          ...
          {/* Nút thêm dòng cho PHÂN VÙNG 1 — chỉ hiện lần đầu */}
          {isFirstTime && (
            <tr>
          ...
          {/* Khoảng cách giữa 2 phân vùng */}
          <tr><td colSpan={10} className="p-1 bg-white border-none" /></tr>

          {/* ══════════ PHÂN VÙNG 2: KẾ HOẠCH TUẦN NÀY ══════════ */}
          <SectionHeader
            icon="🗓️"
            label="Kế hoạch"
            weekStr={planWeek}
